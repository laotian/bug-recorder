...
- 📷 **屏幕截图**: 支持全页面/可视区域/指定元素截图功能，Alt+点击仅记录截图事件
- 📋 **备注功能**: 添加自定义备注信息
- 📄 **Markdown导出**: 自动生成格式化的复现报告
- ⌨️ **快捷键操作**: 支持Ctrl+Shift+Alt+R快捷键
...
#### 1. 真实截图模式（普通点击）
- **操作方式**: 直接点击截图按钮（摄像头图标）
- **功能**: 使用内置的dom-to-image技术进行真实截图
- **输出**: 在录制报告中嵌入完整的base64图片数据
- **适用场景**: 需要在报告中直接查看截图内容的情况

#### 2. 事件记录模式（Alt+点击）
- **操作方式**: 按住Alt键后点击截图按钮
- **功能**: 仅在录制时间线中记录截图事件，不生成实际图片
- **输出**: 在录制报告中显示"用户手动截图"文本标记
- **适用场景**: 使用外部截图工具（如系统截图、专业截图软件）时的时间点标记
...
**使用建议：**
- 对于重要的界面状态变化，使用真实截图模式获得完整的视觉记录
- 对于配合外部截图工具的工作流，使用事件记录模式标记时间点
- 两种模式可以在同一次录制中混合使用
...
2. **执行操作**: 进行需要记录的用户操作
3. **添加截图**: 
   - 普通点击摄像头图标：执行真实截图并保存图像
   - Alt+点击摄像头图标：仅记录截图事件，配合外部截图工具使用
4. **添加备注**: 点击笔形图标添加说明文字
5. **停止录制**: 点击红色停止按钮
...
- ✅ 智能内容过滤：仅记录Content-Type包含application/json的请求，避免字体文件等无关请求
...
- ✅ 智能过滤录制器UI元素，确保截图纯净
- ✅ 支持CSS选择器指定截图区域
- ✅ 双模式截图：普通点击真实截图，Alt+点击仅记录截图事件（配合外部工具）
...
- ✅ **隐藏模式**: 快捷键(Ctrl+Shift+Alt+R)控制显示/隐藏
- ✅ **vConsole集成**: 无缝集成开发者调试工具，完全依赖传入的vConsole实例
